...
/*
 * @author MasterWard and Powell "Hap" Hazzard
 */
public class Prob15 {
     ...
     */
    static class RailwayShunting {
    }

    // class that will represent the Rail Cars
    class RailCar {

        int num; // Railcar Number
...
//        Store the final car location
    ...
    public class Track extends RailwayShunting {

        String name;
        RailCar cars[]; // Queue of RailCars
        int numCars = 0;  // number of current RailCars on this siding
        int maxCars = 0;  // max number allowed on this siding
        public boolean full; // See if siding is full
        public boolean empty;

        public Track(String name, int max) {
            maxCars = max;
        ...
        /* Return if a car is on this siding */
        ...
        /* Compare if 2 tracks are already in order */
    ...
    public Track[] initTracks(String ab, String cd, String results) {
        /* Set the track and max number cars */
        Track a = new Track("A", 5);
        Track b = new Track("B", 5);
        Track c = new Track("C", 3);
        Track d = new Track("D", 3);
        Track finalOrder = new Track("Final", 5); // desired results on "A"
        Track all[] = {a, b, c, d, finalOrder}; // return all tracks

        for (int i = 0; i < a.maxCars; i++) { // load "A" siding
            if (ab.charAt(i) != '-' && ab.charAt(i) != 'L') {
                RailCar n = new RailCar(ab.charAt(i) - '0');
                a.insertCar(n);
            }
        }
        for (int i = 0; i < c.maxCars; i++) { // Load "C" siding
            if (cd.charAt(i) != '-') {
                RailCar n = new RailCar(cd.charAt(i) - '0');
                c.insertCar(n);
            }
        }
        for (int i = ab.length() - 1; ab.charAt(i) != '+'; i--) { // load "B" siding 
            if (ab.charAt(i) != '-') {
                RailCar n = new RailCar(ab.charAt(i) - '0');
                b.insertCar(n);
            }
    ...
    public void printTrackView(Track mainSiding, Track subSiding[]) {
        System.out.println();
        System.out.println(mainSiding.printTrack(false, true) + subSiding[0].printTrack(true, false)); // reversed, not mainline
        System.out.println(subSiding[1].printTrack(false, false) + "^" + subSiding[2].printTrack(true, false));
    }
    ...
    public void orderCars(Track t[]) {
        ...
        wh:
        while (subSiding[0].hasFinalCars() || subSiding[1].hasFinalCars() || subSiding[2].hasFinalCars()) {
                ...
                if (subSiding[i].empty) {
                    ...
                    if (!mainSiding.empty && mainSiding.peekFirstCar().notUsed) {
                        ...
                        for (int i1 = 0; i1 < numSubSiding; i1++) {
                            if (mainSiding.empty && !subSiding[i1].empty && subSiding[i1].peekFirstCar().notUsed) {
            ...
            if (!mainSiding.full && (mainSiding.empty || !mainSiding.peekFirstCar().notUsed)) {
                for (int i = 0; i < numSubSiding; i++) {
                    if (!subSiding[i].empty
            ...
            if (!mainSiding.empty && mainSiding.peekFirstCar().notUsed) {
                for (int i = 0; i < numSubSiding; i++) {
                    if (!subSiding[i].hasFinalCars() && !subSiding[i].full) {
                        subSiding[i].insertCar(mainSiding.removeFirstCar());
                        continue wh;
                    }
                }
            ...
            for (int i = 0; i < numSubSiding; i++) {
                if (subSiding[i].hasFinalCars() && !subSiding[i].peekFirstCar().notUsed) {
        ...
        while (!finalSiding.equal(mainSiding) && !mainSiding.empty
            ...
            for (int i = 0; i < numSubSiding; i++) {
                ...
                if (!subSiding[i].full && (bs == null || ms.posPrio < bs.posPrio || bs.notUsed)) {
                    subSiding[i].insertCar(mainSiding.removeFirstCar());
                    break;
                }
            }
...
//  final stage put them in order...
            ...
            loopCounter++;
            {
                ...
                for (int i = 0; i < numSubSiding; i++) {
                    ...
                    if (next == null & !rc.notUsed) {
                        next = subSiding[i];
                    } else if (!rc.notUsed && rc.posPrio < next.peekFirstCar().posPrio) {
                        next = subSiding[i];
                    }
                }
                if (ms == null && next != null) {
                    mainSiding.insertCar(next.removeFirstCar());
                } else if (ms != null && next != null
    ...
    public static void main(String[] args) throws Exception {
        Prob15 pr = new Prob15();
        /* Read in the input */
        Scanner scan = new Scanner(new File("prob15.txt"));
        String ab = scan.nextLine().trim();
        String cd = scan.nextLine().trim();
        String result = scan.nextLine().trim();
        /* Solves */
        Track tt[] = pr.initTracks(ab, cd, result);
        pr.orderCars(tt);
        pr.printTrackView(tt[0], tt);
    }
}
