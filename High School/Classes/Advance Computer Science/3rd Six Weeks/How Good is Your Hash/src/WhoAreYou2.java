...
//        Scanner scan = new Scanner(new File("sampleperson.txt")); // Sample Scanner
            ...
            wrong.close();
            /**
             * Start actually getting the data
             */
...
//                    out.println(sucRes);
            ...
            for (String xp : peeps) {
                String line[] = xp.split("\t"); // Scans the people in
                q.put(line[3], new Person(line[0], line[1], line[2], line[3])); // Puts in linkedList Hashtable
            }
            endBuild = System.currentTimeMillis();
...
//                    out.println(tem.getNumber() + " " + line[2]);
            ...
            for (String xp : peeps) {
                String line[] = xp.split("\t"); // Scans the people in
                r.put(line[3], new Person(line[0], line[1], line[2], line[3])); // Puts in linkedList Hashtable
            }
            endBuild = System.currentTimeMillis();
            sucRes = 0; // Successful results
            sucRes2 = 0; // Unsuccessful successful results
