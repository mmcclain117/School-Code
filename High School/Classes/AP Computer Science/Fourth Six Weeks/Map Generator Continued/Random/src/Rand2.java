...
public class Rand2 extends JFrame {
...
//        }
    ...
    public Rand2(int r, int c) // takes arguments size of map
    {
        rows = r;
        cols = c;
...
//                    if (!(WATER == map[i][j])) {
                    if (i - 1 > 0) {
                        r *= map[i - 1][j] == DESERT ? 1 + (con * .05) : map[i - 1][j] == FOREST ? 1 + (con * .05) : 1 + (con * .05);
                    } else if (i + 1 < rows) {
                        r *= map[i + 1][j] == DESERT ? 1 + (con * .05) : map[i + 1][j] == FOREST ? 1 + (con * .05) : 1 + (con * .05);
                    }
                    if (j - 1 > 0) {
                        r *= map[i][j - 1] == DESERT ? 1 + (con * .05) : map[i][j - 1] == FOREST ? 1 + (con * .05) : 1 + (con * .05);
                    } else if (j + 1 < cols) {
                        r *= map[i][j + 1] == DESERT ? 1 + (con * .05) : map[i][j + 1] == FOREST ? 1 + (con * .05) : 1 + (con * .05);
                    }
                    if (r < 1.66 || r > 3 && r < 3.5) {
                        map[i][j] = DESERT;
                    } else if (r < 2.66 || r > 4 && r < 5.5) {
                        map[i][j] = FOREST;
                    } else {
                        map[i][j] = WATER;
                    }
