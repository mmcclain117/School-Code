...
/**
 * A pen can be used to draw on a canvas. The pen maintains a position,
 * direction, color, and an up/down state. The pen can be moved across the
 * canvas. If the pen is down, it leaves a line on the canvas when moved. (If it
 * is up, it will not draw a line.)
 *
...
public class Pen {
    ...
    /**
     * Create a new Pen with its own canvas. The pen will create a new canvas
     * for itself to draw on, and start in the default state (centre of canvas,
     * direction right, color black, pen down).
     */
    public Pen() {
    ...
    /**
     * Create a new Pen for a given canvas. The direction is initially 0 (to the
     * right), the color is black, and the pen is down.
     *
    ...
    /**
     * Move to the specified location. If the pen is down, leave a line on the
     * canvas.
     *
    ...
    /**
     * Turn the specified amount (out of a 360 degree circle) clockwise from the
     * current rotation.
     *
    ...
    /**
     * Turn to the specified direction. 0 is right, 90 is down, 180 is left, 270
     * is up.
     *
    ...
    /**
     * Scribble on the canvas in the current color. The size and complexity of
     * the squiggle produced is defined by the constants SQIGGLE_SIZE and
     * SQIGGLE_COUNT.
     */
