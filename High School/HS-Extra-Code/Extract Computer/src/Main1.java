...
//                }
    ...
    private static String deleteWhat(boolean userInput) {
        String act = "";
        if (userInput) {
            Scanner scan = new Scanner(System.in);
            out.println("Delete\nAll, Moved, None");
            String x = scan.nextLine();
            while (!(x.equalsIgnoreCase("All") || x.equalsIgnoreCase("Moved") || x.equalsIgnoreCase("None"))) {
                out.println("That is not a choice");
                out.println("All, Moved, None");
                x = scan.nextLine();
            }
            act = x;
        } else {
