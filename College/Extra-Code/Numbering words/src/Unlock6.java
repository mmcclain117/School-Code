...
public class Unlock6 {
	...
	public static void main(String[] args) throws Exception {
		...
		scan.close();
		cusObj co[] = new cusObj[1000000];
		for (String s : ar) {
			int temp = Integer.parseInt(transfer(s));
			if (temp == 111111 || temp == 222222 || temp == 333333 || temp == 444444 || temp == 555555 || temp == 666666
					|| temp == 777777 || temp == 888888 || temp == 999999) {
		...
		comon.close();
		FileWriter go = new FileWriter(new File("output6Pass.txt"));
		List<cusObj> temps = Arrays.asList(co);
		Collections.shuffle(temps);
		Object sco[] = temps.toArray();
		co = (cusObj[]) sco;
		String x = "";
//		int i = 0; // Used to control the for loop
//		for (cusObj s : co) {
		for(int i =0;i < co.length;i++) { // I is used as control
			cusObj s = co[i];
			if (s == null) {
				// Fix if it is empty
			} else if (s.good() && i < co.length / 100) {
