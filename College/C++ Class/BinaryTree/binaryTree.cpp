...
Tree *succesorInOrder(struct Tree *node) {
	/* if the node has right child, successor is Tree-Minimum */
	if (node->right != NULL)
...
/* 1. Traverse the left subtree.
 ...
 3. Visit the root.
 */
void postTraversal(struct Tree *node) {
	if (node != NULL) { // There is a tree
...
/* 1. Visit the root.
 ...
 3. Traverse the right subtree.
 */
void preTraversal(struct Tree *node) {
	if (node != NULL) { // There is a tree
		cout << node->data << " ";
...
/* In reverse of inOrderTraversal() */
...
void BreadthFirst_LevelElement_Print(struct Tree *root,
		vector<vector<int> > &v) {
	if (root == NULL)
		return;
	deque<Tree *> q;
...
void level_even_odd(struct Tree *node) {
	...
	for (unsigned int i = 0; i < evenVec.size(); i++)
		cout << evenVec[i] << " ";
	cout << endl << "odd level elements : ";
	for (unsigned int i = 0; i < oddVec.size(); i++)
		cout << oddVec[i] << " ";
	cout << endl;
}
...
//	char charArr[9] = { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I' };
	char mixArr[] = { 'B', 'A', 'D', 'C', 'E', 'G', 'I', 'H' };
	Tree *root = newTreeNode('F');
	/* Populates the Tree */
	for (unsigned int i = 0; i < sizeof(mixArr); i++) {
		insertNode(root, mixArr[i]);
	}

	isBST(root); // Is it a BST?
	...
	for (unsigned int m = 0; m < levVec.size(); m++) {
		cout << "Level at " << m << ": ";
		for (unsigned int n = 0; n < levVec[m].size(); n++)
	...
	for (int i = 0; i <= mxDepth; i++) {
		cout << "level " << i << ": ";
		for (unsigned int j = 0; j < elm[i].size(); j++)
			cout << elm[i][j] << " ";
		cout << endl;
	}
...
//	cout << "First time " << (float) t1 << endl;
//	cout << "Second time " << t2 << endl;
//	cout << (diff / (double) CLOCKS_PER_SEC * 1000.0) << endl;
//	system("pause");
	return 0;
