...
WCS_UB_Array<DATA, INDEX>::Node::Node(const INDEX & I, size_t & NumElements) :
		Index(I) {
	NumElements++;
}
...
template<class DATA, class INDEX>
inline typename WCS_UB_Array<DATA, INDEX>::Exceptions WCS_UB_Array<DATA, INDEX>::Remove(
		const INDEX & I) {
	pCurrent = pHead;
...
DATA & WCS_UB_Array<DATA, INDEX>::operator [](const INDEX & I) {
	...
	if (pHead.DoesNotExist())	// is the list empty
	{
		...
		pTail = pHead;
		return (*pHead).Data;
	} else if (I < (*pHead).Index)// is the index in front of the existing list
			{
		...
		pHead = pNode;
		return (*pNode).Data;
	} else if (I > (*pTail).Index) // is the index after the existing list
			{
		pNode = WCS_Pointer<Node>(new Node(I, NumElements));
		(*pNode).pPrev = pTail;
		(*pTail).pNext = pNode;
		pTail = pNode;
		return (*pNode).Data;
	} else if (I == (*pHead).Index)	// is the Index the first thing in the list
		return (*pHead).Data;
	else if (I == (*pTail).Index)	// is the Index the last element
		return (*pTail).Data;
	else {
		...
		{
			WCS_Pointer<Node> pNode2(
					WCS_Pointer<Node>(new Node(I, NumElements)));
			(*pNode2).pNext = pNode;
...
const DATA & WCS_UB_Array<DATA, INDEX>::operator [](const INDEX & I) const {
	...
	if (pHead.DoesNotExist())	// is the list empty
		...
		if (I == (*pNode).Index) // Is this the node I want
			...
			// This is the node after where the new node needs to go
			throw IndexDoesNotExist;
	}
}
