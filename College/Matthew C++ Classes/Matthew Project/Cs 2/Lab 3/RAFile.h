...
public:
	...
	Exceptions	Insert(UINT Index, const string & Data)
	{
		...
		else
		{
			...
			{
				//There is a bug in this function that sometimes causes the record at Index and Index + 1 to be replaced with Data if the requested insertion index is 0 or 1.
				for (i = NumRecords - 1; i >= Index; i--)//Index cannot be 0 when this executes or there will be an infinite loop.
				...
				if (Data.length() > RecInfoArray[Index].Length)	// is there enough space where the record is now
				{
					...
					cout << RecInfoArray[Index].Length << endl;
					NumRecords++;
